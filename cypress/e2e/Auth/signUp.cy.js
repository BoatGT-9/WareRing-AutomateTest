...
describe("SignUp", () => {
  ...
  it("TC 1001 sign up to webpage success fully", function () {
    cy.visit(BASE_URL);

    cy.get("#toggle-big-register-landing-drawer-btn").click();
    cy.get("#username").type(`userName${new Date().getSeconds()}`);
  ...
  // signUp fail All
  // ชื่อ user ซ้ำ
  it("TC 1002 sign up to fail username to used", () => {
    cy.visit(BASE_URL);
  ...
  // รหัสผ่านง่ายเกินไป
  it("TC 1003 sign Up to fail password not match so Ez ", () => {
    cy.visit(BASE_URL);
    cy.get("#toggle-big-register-landing-drawer-btn").click();
    cy.get(" #username").type(`${user1.username}`);
    cy.get(" #firstName").type(`${user1.firstName}`);
  ...
  // ชื่อผู้ใช้ เกิน 30  ตัว
  it("TC 1004 sign up to fail username not match pattern and over 30 characters", () => {
    cy.visit(BASE_URL);
    cy.get(`#toggle-big-register-landing-drawer-btn`).click();
  ...
  it("TC 1005 sign up to fail username not match pattern", () => {
    cy.visit(BASE_URL);
    cy.get(`#toggle-big-register-landing-drawer-btn`).click();
    ...
    // cy.wait(10000);
    cy.get("#setup-user-drawer-submit").click();
    cy.contains("username must match ").should(
      "exist"
    );
    cy.screenshot();
  });
  // ชื่อจริงไม่ตรงกับรูปแบบ
  it("TC 1006 sign up to fail firstName not match pattern", () => {
    cy.visit(BASE_URL);
    cy.get(`#toggle-big-register-landing-drawer-btn`).click();
    cy.get(" #username").type(`usernameTest`);
    ...
    cy.get("#setup-user-drawer-submit").click();
    cy.contains(
      "firstName must match"
    ).should("exist");
    cy.screenshot();
  });

  // password กับ confirmPassword ไม่ตรงกัน
  it("TC 1007 sign up to fail password not match", () => {
    cy.visit(BASE_URL);
    ...
    cy.get(" #email").type(`emailTest${new Date().getSeconds()}@test.com`);
    cy.get(" #password").type(`Passwrod1234`);
    cy.get(" #confirm_password").type(`Passwrod123433`);
    cy.get("#agree-term-and-conditions-drawer-checkbox").click();
    // cy.wait(10000);
    cy.get("#setup-user-drawer-submit").click();
